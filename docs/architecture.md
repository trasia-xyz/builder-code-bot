...
可恢复状态，并通过本地进程锁、不可变 manifest 和链上证据避免重复付款。
...
| `internal/repository/mysql` | 数据查询、完成更新、提交结果复核和长期重试 |
...
派生地址一致。服务不会把私钥或解密密码写入日志、通知或本地状态，也不会把签名或
完整签名请求写入日志和通知。为了支持 exact-request 恢复，本地状态会保存完整签名
请求及其 hash；因此 `./data` 虽不包含私钥，仍必须作为敏感数据保护。
...
```sql
SELECT id, period_start_at, amount
...
1. 解析和严格校验配置。
2. 初始化日志和 SES 通知。
3. 从配置读取共享密码，或在真实 TTY 中无回显读取一次，然后解密并验证全部 builder
   与 settlement 私钥地址。
4. 初始化 MySQL 连接池。
5. 获取 `./data/LOCK`。
6. 初始化 Hyperliquid clients 和 orchestrator。
7. 如果存在 current state，先执行恢复。
8. 如果显式提供 `--run-on-start`，再执行一轮新任务；否则等待下一 UTC 00:00。
9. 每轮结束后重新计算下一 UTC 零点。
...
环境变量读取该密码。KDF 派生 key、终端输入 buffer、解密 buffer，以及私钥解析产生的
临时字节和 scalar 会在使用后尽力清零。运行期签名对象必须在服务生命周期内保留；
同时 Go 字符串和运行时复制不提供完整的安全内存擦除保证，因此内存清零只作为纵深
防御，不能替代主机隔离、最小权限和进程生命周期控制。
...
日志支持 console 或 JSON、动态 level、可选颜色和 source。业务流程以结构化事件记录
必要的运行上下文，具体字段可以随可观测性需求演进。稳定的安全约束是敏感属性会在
handler 层过滤，日志不得包含私钥、解密密码、签名或完整签名请求。
...
- MySQL 数分钟 outage、commit ambiguity 和恢复后的幂等完成；
